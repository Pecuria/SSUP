...
def areaForPoly(verts):
    ...
    pmv = [pm.Vec2d(v[0],v[1]) for v in list(verts)] #not pm.vec2d(v) because v need to be (x,y)
...
def centroidForPoly(verts):
    ...
    #for v in list(verts):
    #    print(v)
    pmv = [pm.Vec2d(v[0],v[1]) for v in list(verts)]
    #pmv = [*map(lambda v: pm.Vec2d(v), verts)]
    for i in range(len(pmv)):
        v1 = pmv[i]
        v2 = pmv[(i+1) % len(pmv)]
...
def segs2Poly(seglist, r):
    vlist = [pm.Vec2d(v[0],v[1]) for v in seglist]
